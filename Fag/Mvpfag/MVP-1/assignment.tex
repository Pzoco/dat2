...
\begin{itemize}
	...
	\item Race condition: A race condition occurs when two threads race to influence the output first. If the output of something has been changed by another thread at a time where it should or shouldn't have a race condition may arise.
	...
	\item Lock Contention: This occurs when two threads tries to acquire a lock at the same time. They are contending for the lock in the theoretical situation of reaching it at the same time. Does not happen often in practice but must still be considered for stability.
	\item Granularity: Is the size of the subproblems. When dividing a program into smaller problem we say the granularity larger if each subproblem is larger ie. the problem is more "coarse"
...
MSize            & Last Version	& This version	& Improvement (\%) \\
\hline
100 			& 0.008409s		& 0.007974s		& $\sim$ 5,45 \\
200            	& 0.061398s		& 0.062984s		& $\sim$ -2.51 \\
1000     		& 11.7579s		& 7.76633s		& $\sim$ 51	\\
\end{tabular} \\
